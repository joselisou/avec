...
|---|---|
| `token` | JWT Bearer, usado no header `authorization` de toda chamada a `api.avec.beauty` (~7 dias de validade) |
...
Todas as chamadas usam header `authorization: <token>` e `accept: application/json`. Base: `https://api.avec.beauty`.
