<?php
/**
 * Imports a WXR file into the Avec Clone post types, upserting by `_avec_source_id` so a file
...
/**
 * Class Avec_Clone_Importer.
 */
class Avec_Clone_Importer {

	const SOURCE_ID_META = '_avec_source_id';
	...
	public static function import_file( $file_path ) {
		...
		foreach ( $xml->channel->item as $item ) {
			...
			if ( null === $source_id ) {
				continue; // Not an Avec Clone item (no source id), skip.
			}
	...
	/**
	 * Reads the `_avec_source_id` value out of an item's `<wp:postmeta>` entries.
	 *
	...
	/**
	 * Finds an existing post by `(post_type, _avec_source_id)` and updates it, or inserts a new one.
	 *
	 * @param SimpleXMLElement $item      The `<item>` element.
	 * @param SimpleXMLElement $wp_fields The item's `wp:` namespaced children.
	 * @param string           $post_type Target post type.
	 * @param string           $source_id Value of `_avec_source_id` for this item.
	...
	/**
	 * Looks up a post by post type and `_avec_source_id` meta value.
	 *
	...
	/**
	 * Replaces every `_avec_*` postmeta on a post with the values from its WXR item.
	 *
	...
	/**
	 * Second pass: resolves every `_avec_*_source_id` meta (e.g. `_avec_cliente_source_id`) into
	 * the matching post's real ID (`_avec_cliente_post_id`). Runs after every item has been
	...
	private static function resolve_relations() {
		...
		$like = $wpdb->esc_like( '_avec_' ) . '%' . $wpdb->esc_like( '_source_id' );
		...
		foreach ( $rows as $row ) {
			...
			// Turns e.g. meta key "_avec_cliente_source_id" into the relation name "cliente",
			// so it can be resolved into the target post's id under "_avec_cliente_post_id".
			$relation   = str_replace( array( '_avec_', '_source_id' ), '', $row->meta_key );
			...
			if ( $target_ids ) {
				update_post_meta( $row->post_id, "_avec_{$relation}_post_id", (int) $target_ids[0] );
			}
		}
	}
}
