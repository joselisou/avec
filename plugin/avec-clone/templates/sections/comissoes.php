...
<?php
$avec_filter_extra_html = ob_get_clean();
?>
<main class="avec-clone-section avec-clone-section--comissoes">
	...
	<?php
	$avec_servicos_url = add_query_arg(
		array(
			'inicio' => $avec_filter_inicio,
			'fim'    => $avec_filter_fim,
			'recibo' => $avec_filter_recibo,
		),
		Avec_Clone_Frontend_Router::url_for_sub_route( 'comissoes/servicos' )
	);
	?>
	<a class="avec-clone-panel avec-clone-panel--link" href="<?php echo esc_url( $avec_servicos_url ); ?>">
	 ...
	 */
	if ( apply_filters( 'avec_clone_show_comissoes_lancamentos', false ) ) :
		?>
		<?php if ( $recibos ) : ?>
